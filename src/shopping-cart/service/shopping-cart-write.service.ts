...
export class ShoppingCartWriteService {
    ...
    async create(shoppingCart: ShoppingCart): Promise<UUID> {
        return await this.#tracer.startActiveSpan(
            'shopping-cart-service.write.create-cart',
            async (outerSpan) => {
                try {
                    return await otelContext.with(
                        ...
                        async () => {
                            this.#logger.debug(
                                'create: shoppingCart=%o',
                                shoppingCart,
                            );

                            const shoppingCartDb =
                                await this.#tracer.startActiveSpan(
                                    'shopping-cart-repository.save',
                                    async (span) => {
                                        try {
                            ...
                            this.#logger.debug(
                                'create: shoppingCartDb=%o',
                                shoppingCartDb,
                            );

                            const trace =
                                this.#traceContextProvider.getContext();
            ...
            async (span) => {
                try {
                    return await otelContext.with(
                        ...
                        async () => {
                            this.#logger.debug(
                                'addItem: item=%o, username=%s',
                                newItem,
                                customerId,
                            );

                            const shoppingCart =
                                await this.#readService.findByCustomerId({
                                    customerId,
                                });
                ...
                try {
                    return await otelContext.with(
                        ...
                        async () => {
                            ...
                            if (!item) {
                                this.#logger.warn(
                                    `removeItem: Item mit ID ${id} nicht gefunden`,
                                );
                                throw new NotFoundException(
                                    `Es gibt kein Item mit der ID ${id}.`,
                                );
                            }
                            ...
                            this.#logger.debug(
                                'removeItem: erfolgreich entfernt',
                            );
                            return true;
                        },
                    );
            ...
            async (span) => {
                try {
                    return await otelContext.with(
                        ...
                        async () => {
                            this.#logger.debug(
                                'orderItems: inventoryIds=%o, customerId=%s',
                                inventoryIds,
                                customerId,
                            );
                    ...
                    return await otelContext.with(
                        ...
                        async () => {
                            ...
                            if (id === undefined) {
                                ...
                                throw new NotFoundException(
                                    `Es gibt kein ShoppingCart mit der ID ${id}.`,
                                );
                            }
                            ...
                            const merged = this.#shoppingCartRepository.merge(
                                shoppingCartNeu,
                                shoppingCart,
                            );
                            this.#logger.debug('update: merged=%o', merged);
                            const updated =
                                await this.#shoppingCartRepository.save(merged); // implizite Transaktion
     ...
     */
    async delete({ customerId }: { customerId?: UUID }): Promise<boolean> {
        return await this.#tracer.startActiveSpan(
                ...
                try {
                    return await otelContext.with(
                        ...
                        async () => {
                            ...
                            this.#logger.debug(
                                'delete: shoppingCart=%o',
                                shoppingCart,
                            );

                            let deleteResult: DeleteResult | undefined;

                            await this.#shoppingCartRepository.manager.transaction(
    ...
    async #validateUpdate(
        ...
        this.#logger.debug(
            '#validateUpdate: shoppingCartDb=%o',
            shoppingCartDb,
        );
