...
export class ShoppingCartReadService {
    ...
    constructor(
        ...
        this.#logger = this.#loggerService.getLogger(ShoppingCartReadService.name);
        this.#tracer = trace.getTracer('shopping-cart-read-service');
    }

    async findById({ id, withItems = true }: FindByIdParams) {
        return await this.#tracer.startActiveSpan('shopping-cart-service.read.find-by-id', async (outerSpan) => {
            try {
    ...
    */
    ...
    /**
         ...
         */
    async #calculateCartSummary(shoppingCart: ShoppingCart): Promise<{ totalAmount: Decimal; isComplete: boolean }> {
        this.#logger.debug('#calculateCartSummary: shoppingCart=%o', shoppingCart);

        const totalAmountDecimal = shoppingCart.cartItems?.reduce((sum, item) => {
