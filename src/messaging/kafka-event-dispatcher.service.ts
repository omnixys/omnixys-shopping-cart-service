...
export class KafkaEventDispatcherService implements OnModuleInit {
    ...
    readonly #handlerMap: Map<string, (topic: string, data: unknown, context: KafkaEventContext) => Promise<void>> = new Map();
    ...
    onModuleInit(): void {
        ...
        for (const wrapper of providers) {
            ...
            for (const methodName of methodNames) {
                ...
                const topics: string[] = this.#reflector.get(KAFKA_EVENT_TOPICS, method) ?? [];

                for (const topic of topics) {
                    this.#logger.debug(`📩 Registriere Topic "${topic}" für ${instance.constructor.name}.${methodName}()`);

                    this.#handlerMap.set(topic, async (t, d, c) => {
                        try {
                            await method.call(instance, t, d, c);
                        } catch (err) {
                            this.#logger.error(`Fehler beim Aufruf von ${instance.constructor.name}.${methodName}:`, err);
                        }
                    });
                }
            }
    ...
    async dispatch(topic: string, payload: any, context: KafkaEventContext): Promise<void> {
        this.#logger.debug('dispatch: topic=%s, payload=%o, context=%o', topic, payload, context);
        ...
        try {
            ...
            this.#logger.error(`Fehler beim Verarbeiten von Topic "${topic}":`, error);
        }
    }
}
